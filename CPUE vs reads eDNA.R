...
PlotMMito.efish = ggplot(merged.MMito.efish, aes(x = CPUE, y = stand.reads)) +
  geom_point(aes(color = Capture.Method), colour = "red") +  
...
PlotMMito.seine = ggplot(merged.MMito.seine, aes(x = CPUE, y = stand.reads)) +
  geom_point(aes(color = Capture.Method), colour = "green") +  
...
hist(resids.betaMito)
qqnorm(resids.betaMito)
...
hist(resids.betaMito.od)
qqnorm(resids.betaMito.od)
...
final.predict.Mito = cbind(predict.values,predicts)
...
hist(resids.betaPS1.od)
qqnorm(resids.betaPS1.od)
...
final.predict.PS1 = cbind(predict.values,predicts)
