...
# log.Mean.eDNA.scaled 4.24         2.06      0.24

#Moderate Correlation

#                   Term  VIF Increased SE Tolerance
#                    Year 5.98         2.44      0.17
# Mean.Temperature.scaled 6.89         2.62      0.15
...
for(j in unique(full.dataset$Lake)){
  ...
  x = simulate(glm.nb.sim,nsim = round(1000/length(sub$eDNA), digits = 0) ) #puts it into a matrix 
...
write.csv(final.sim.nb.dataset, file = "C:/Users/matth/Dropbox/Maxime Bernatchez Project/final.sim.nb.dataset.csv")
write.csv(summary.sim.nb.dataset, file = "C:/Users/matth/Dropbox/Maxime Bernatchez Project/summary.sim.nb.dataset.csv")
...
Breaks2 = seq(0,200,20)
hist(summary.sim.nb.dataset$sample.size.90.within.10perc,main = c(paste("Sampling effort required to obtain "),paste("90% of estimates within +/- 20% of mean")), xlab = "Sampling Effort", ylab = "Frequency", breaks = Breaks2)
